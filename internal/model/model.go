...
// BUG8 fault: zero values are treated as explicit replacements.
func (t *Task) ApplySchedulePatch(p SchedulePatch) {
	t.SpecType = p.SpecType
	t.CronExpr = p.CronExpr
	t.DelaySeconds = p.DelaySeconds
	t.NextRunAt = p.NextRunAt
}
